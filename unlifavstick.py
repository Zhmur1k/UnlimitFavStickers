...
TRANSLATIONS = {
    ...
    "uk": {
        ...
        "bulletin_sub": "При додаванні та видаленні стікера з улюблених",
...
def get_app_language() -> str:
    """Gets the current language dynamically from Telegram's LocaleController."""
    try:
        LocaleController = find_class("org.telegram.messenger.LocaleController")
        if LocaleController:
            instance = LocaleController.getInstance()
            
            # 1. Try getCurrentLocaleInfo() for Telegram specific shortName (ru, en, uk)
            try:
            ...
            # 2. Fallback to standard Java getCurrentLocale()
            try:
        ...
        log(f"unlifavstick lang detection error: {e}")
        
    return "en" # Fallback globally

def tr(key: str, *args) -> str:
    """Returns the translated string based on current app language."""
